...
export default function Card(project: projectDataInterface) {
    const className = `cursor-pointer border-0
    ...
    return (
        <div className="bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.5)] rounded-lg flex flex-col">
            ...
            <div className="p-4 flex flex-col h-full">
                ...
                <p className="text-justify font-normal text-gray-400">{project.description}</p>
                ...
                <div className="mt-auto flex justify-between">
                    <Button content="Github" variant="secondary" icon={(<GitBranch size={22} />)} onClick={() => handleClick(project.repository)} />
                    {project.link &&
                        (
                            <Button content="Abrir" variant="primary" icon={(<Link size={22} />)} onClick={() => handleClick(project.link)} />
                        )
                    }
                </div>
            </div>
